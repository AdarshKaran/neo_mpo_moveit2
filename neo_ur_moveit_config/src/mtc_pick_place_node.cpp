...
public:
  ...
  /* ---- full initialisation of the pick-and-place pipeline ---- */
  void initializeTask()
  {
    setupPlanningScene();
    createTask();
...
//   }
  ...
  void createTask()
  {
    ...
    /* ---------------- open hand ---------------- */
    {
      auto s = std::make_unique<mtc::stages::MoveTo>("open hand",
                                                     interpolation_planner);
      s->setGroup(hand_group_name_);
      s->setGoal(open_pose_);
      task->add(std::move(s));
    }
    ...
    /* ========== PLACE SERIAL CONTAINER ========== */
    {
      ...
      /* sample place pose + IK */
      {
        ...
        tgt.pose.position.z = place_pose_z_ + 0.025;  // Add object height
      ...
      /* forbid collision hand to object */
      {
        auto s = std::make_unique<mtc::stages::ModifyPlanningScene>(
